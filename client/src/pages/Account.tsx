...
export default function Account() {
  ...
  const [usernameStatus, setUsernameStatus] = useState<StatusKind>("idle");
  ...
  }, [user?.username, username, usernameSubmitting]);

  const passwordReady = useMemo(() => {
    return (
      (user?.auth_provider === "google" || currentPassword.trim().length > 0) &&
      newPassword.trim().length >= 8 &&
      confirmPassword.trim().length >= 8 &&
      newPassword === confirmPassword &&
      !passwordSubmitting
    );
  }, [confirmPassword, currentPassword, newPassword, passwordSubmitting, user?.auth_provider]);
  ...
  const isGoogleAuthUser = activeUser.auth_provider === "google";
  ...
  async function handleUsernameSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await readErrorMessage(res, "Failed to update username"));
      }
      ...
      setUsernameStatus("error");
      setUsernameMessage(error instanceof Error ? error.message : "Failed to update username");
    } finally {
  ...
  async function handlePasswordSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const passwordEndpoint = isGoogleAuthUser ? "/auth/password/set-initial" : "/auth/password";
      ...
      setPasswordMessage(isGoogleAuthUser ? "Password set successfully." : "Password updated successfully.");
      setCurrentPassword("");
      setNewPassword("");
      setConfirmPassword("");
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="grid items-stretch gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="flex h-full min-h-[292px] flex-col rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-700 p-6 text-white shadow-sm">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-12">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4 md:col-span-6">
              <div className="text-xs uppercase tracking-wide text-slate-300">Email</div>
              <div
                className="mt-2 overflow-hidden text-ellipsis whitespace-nowrap text-lg font-semibold"
        ...
        <section className="flex h-full min-h-[292px] flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-6 grid items-stretch gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="h-full rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-slate-900">Update username</h2>
          ...
          <form className="mt-6 flex flex-1 flex-col gap-4" onSubmit={handleUsernameSubmit}>
            ...
            <StatusBanner kind={usernameStatus} message={usernameMessage} />
        ...
        <section className="flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-slate-900">{isGoogleAuthUser ? "Set password" : "Change password"}</h2>
              <p className="text-sm text-slate-500">
                {isGoogleAuthUser
          ...
          <form className="mt-6 flex flex-1 flex-col gap-4" onSubmit={handlePasswordSubmit}>
            {!isGoogleAuthUser && (
              <label className="block">
            ...
            <div className="mt-auto">
              <button
                ...
                className="inline-flex items-center justify-center rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {passwordSubmitting ? (isGoogleAuthUser ? "Setting..." : "Updating...") : (isGoogleAuthUser ? "Set password" : "Update password")}
              </button>
            </div>
          </form>
        </section>

        <section className="rounded-3xl border border-rose-200 bg-rose-50 p-6 shadow-sm lg:col-span-2">
          <div className="flex items-start gap-3">
